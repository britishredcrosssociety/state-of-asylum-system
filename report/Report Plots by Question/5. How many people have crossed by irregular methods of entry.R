...
SmallBoatNat |>
  ...
  geom_text(aes(label = scales::comma(Total)), show.legend = FALSE, size = rel(3), hjust = 1.1, colour = brc_colours$black_shadow) +
  coord_flip() +
  theme_brc() +
  theme(axis.text.x = element_text(vjust = 0.5, hjust=0.5)) +
  labs(title = "Number of people detected crossing the channel in small boats at March 2023",
...
smallboat |>
  ...
  theme_brc() +
  labs(title = "Number of people crossing the Channel by small boats by age and sex from 2018 to 2022",
       x = "Sex", 
       y = "Number of people detected",
...
Smallboatbyquarter |>
  ...
  geom_bar(position = "stack", stat = "identity") +
  theme_brc() +
  labs(title = "Number of people crossing the Channel by quarter from 2018 to 2022", 
...
irregular_migration |>
  ...
  labs(title =  "Number of people detected by 'irregular' methods of entry from March 2018 to March 2023",
       ...
       caption = "British Red Cross analysis of Home Office data, March 2018 to March, 2023") +
  scale_y_continuous(labels = scales::comma, limits = c(0, 50000)) +
  scale_colour_manual(values = c(brc_colours$steel,
                                 brc_colours$teal,
                                 brc_colours$red_light,
                                 brc_colours$red_dunant))
